...
func Run(ctx context.Context, opts Options) []Check {
	...
	} else if err := modelCfg.Validate(modelName); err != nil {
		checks = append(checks, fail("model", err.Error()))
	} else {
		checks = append(checks, ok("model", fmt.Sprintf("%s requests %s via provider %s", modelName, modelCfg.RequestID(modelName), modelCfg.Provider)))
	}
...
func checkAPIKey(modelName string, modelCfg config.Model, modelOK bool, apiKey string) Check {
	...
	return ok("api key", fmt.Sprintf("not required for %s", modelName))
}
...
func checkTools(cfg config.ToolConfig) []Check {
	...
	if !info.IsDir() {
		return append(checks, fail("workspace root", fmt.Sprintf("%s is not a directory", root)))
	}
...
func checkSkills(registry *skills.Registry) Check {
	...
	if len(diagnostics) == 0 {
		...
		return ok("skills", fmt.Sprintf("%d discovered (%d model-visible, %d manual-only)", len(all), len(visible), len(all)-len(visible)))
	}
	...
	detail := fmt.Sprintf("%d discovered; %d diagnostics (%d errors, %d warnings)", len(all), len(diagnostics), errors, warnings)
